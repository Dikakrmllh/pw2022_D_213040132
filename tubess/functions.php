...
function koneksi()
{
  $conn = mysqli_connect('localhost', 'root', '','db_store') or die('Koneksi ke DB GAGAL!');
  mysqli_select_db($conn, 'db_store') or die("Database salah!");
  return $conn;
}
...
function tambah($data)
{
  ...
  $query = "INSERT INTO 
                asus_data 
             VALUES
             (null, '$nama', '$gambar', '$spesifikasi', '$harga')
            ";
...
function hapus($id)
{
  ...
  $db_store = query("SELECT * FROM asus_data WHERE id = $id")[0];

  // hapus data gambar
  if ($db_store["gambar"] !== 'nophoto.jpg') {
    unlink('img/' . $db_store["gambar"]);
  }
